import numpy as np
import translator_utils as utils
...
class TranslatorModel(object):
    ...
    def decode(self, x, c, y=None):
        ...
        if (y != None):
            ...
            while y[-1] != word2vec_target[utils.sentence_end_token] and len(y)<30 :
                s.append(sigmoid(self.Wd[:, y[t - 1]] + np.dot(self.Ud, s[t - 1]) + np.dot(self.V, c)))
                o.append(softmax(np.dot(self.P, s[t])))
                
                trial = word2vec_target[utils.unknown_token]
                while(trial == word2vec_target[utils.unknown_token]):
...
if LOAD_MODEL:
    model_file = '822781.model'
    with open(model_file, 'rb') as f:
        model = pickle.load(f)
else:
    model = TranslatorModel(utils.vocabulary_size, utils.vocabulary_size, 50)
    model.train(train_en[:3000],train_fr[:3000])
    pickle.dump(model, open("{}.model".format(datetime.now().strftime("%f")), "wb"))

#trial1 = model.translate(["my name is Kunwar"])
trial1 = model.translate(train_en[10:11])
